...
export default function ContactForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Google reCAPTCHA V3 Token Al
    try {
      const token = await new Promise((resolve, reject) => {
        if (typeof window !== "undefined") {
          window.grecaptcha
            .execute(process.env.NEXT_SITE_KEY, { action: "submit" })
            .then((token) => resolve(token))
            .catch((err) => reject("reCAPTCHA token alınamadı"));
        }
      });

      // Form verileriyle birlikte reCAPTCHA token'ı gönder
      const res = await emailjs.send(
        ...
        { ...formData, recaptchaToken: token }, // reCAPTCHA token'ı dahil et
        process.env.NEXT_PUBLIC_EMAILJS_PUBLIC_ID // EmailJS Public Key
      );
